...
def show_mined_patches(image_id, class_ids, dataloader, hardnegdata):
    ...
    for a in hardnegdata:
        ...
        labels_one_image.append(
            a["label_local"] * (-1 if a["role"] == "neg" else 1))
    ...
    transform_corners_one_image = torch.stack(
        transform_corners_one_image, dim=0)
    # show
...
def show_class_heatmaps(image_id, class_ids, image_fm_sizes_p, class_targets_pyramid, image_class_scores_pyramid, cfg_local,
    ...
    if image_id in cfg_local.images_for_heatmaps:
        for cur_im, label_id in zip(cfg_local.images_for_heatmaps, cfg_local.labels_for_heatmaps):
            ...
            print("Showing scores for image {0}, class {1} (local {2})".format(
                image_id, label_id, i_label))
            ...
            for i_p in range(len(image_fm_sizes_p)):
                if not class_image_augmentation:
                    ...
                    raise RuntimeError(
                        f"Unknown value of class_image_augmentation: {class_image_augmentation}")

                targets = class_targets_pyramid[i_p][i_label *
                                                     num_class_views: (i_label + 1) * num_class_views]
                score_map = image_class_scores_pyramid[i_p][i_label *
                                                            num_class_views: (i_label + 1) * num_class_views]
                # height, width
                shape = (image_fm_sizes_p[i_p].h, image_fm_sizes_p[i_p].w)
                ...
                fig.suptitle(
                    f"Label {label_id}, shape {shape}, level {i_p}, max {score_map.max().item():0.2f}, min {score_map.min().item():0.2f}")

                for i_view in range(num_class_views):
                    axes_target = axes[i_view,
                                       0] if num_class_views > 1 else axes[0]
                    axes_scores = axes[i_view,
                                       1] if num_class_views > 1 else axes[1]
                    show_data(
                        axes_target, targets[i_view], shape, vmin=-1, vmax=1)
                    axes_target.set_title("Targets")
                    show_data(axes_scores, score_map[i_view], shape, vmin=score_map[i_view].min(
                    ).item(), vmax=score_map[i_view].max().item())
                    axes_scores.set_title(
                        f"Scores: min={score_map[i_view].min().item():0.2f}, max={score_map[i_view].max().item():0.2f}")
...
def show_target_remapping(images, class_targets, cls_targets_remapped,
    ...
    gradients_wrt_scores = torch.autograd.grad(
        losses["loss"], class_scores, retain_graph=True)[0]
    gradients_wrt_scores_transform_detached = torch.autograd.grad(
        losses["loss"], class_scores_transform_detached, retain_graph=True)[0]
    ...
    for i_image in range(num_images):
        for i_label in range(num_labels):
            ...
            grad_per_anchor_transform_detached = gradients_wrt_scores_transform_detached[
                i_image, i_label]

            ious_corrected_max_gt = ious_anchor_corrected[i_image, i_label]
            ious_max_gt = ious_anchor[i_image, i_label]

            print("Image {0}, Label {1}: score min {2:0.2f}, score max {3:0.2f}, grad min {4}, grad max {5}, grad TD min {6}, grad TD max {7}".format(i_image, i_label, scores.min(
            ), scores.max(), grad_per_anchor.min(), grad_per_anchor.max(), grad_per_anchor_transform_detached.min(), grad_per_anchor_transform_detached.max()))
            shape = (38, 38)
            ...
            show_data(axes[0][2], scores, shape,
                      vmin=scores.min().item(), vmax=scores.max().item())
            axes[0][2].set_title("Scores, min {0:0.2f}, max {1:0.2f}".format(
                scores.min().item(), scores.max().item()))

            show_data(axes[1][2], loss_per_anchor, shape,
                      vmin=0, vmax=loss_per_anchor.max().item())
            axes[1][2].set_title("Losses, min {0:0.2f}, max {1:0.2f}".format(
                loss_per_anchor.min().item(), loss_per_anchor.max().item()))

            show_data(axes[0][3], grad_per_anchor, shape, vmin=grad_per_anchor.min(
            ).item(), vmax=grad_per_anchor.max().item())
            axes[0][3].set_title("Grads with tr, min {0:0.2f}, max {1:0.2f}".format(
                grad_per_anchor.min().item(), grad_per_anchor.max().item()))

            show_data(axes[1][3], grad_per_anchor_transform_detached, shape, vmin=grad_per_anchor_transform_detached.min(
            ).item(), vmax=grad_per_anchor_transform_detached.max().item())
            axes[1][3].set_title("Grads tr detached, min {0:0.2f}, max {1:0.2f}".format(
                grad_per_anchor_transform_detached.min().item(), grad_per_anchor_transform_detached.max().item()))
    plt.show()
...
def decode_scores_show_detections(dataloader, images, class_ids,
    ...
    for i_image in range(num_images):
        ...
        show_annotated_image(img=dataloader.unnorm_image(images[i_image]),
                             ...
                             default_boxes=boxes_one_image.get_field(
                                 "default_boxes"),
                             transform_corners=boxes_one_image.get_field(
                                 "transform_corners"),
                             class_ids=class_ids,
...
def show_detection_from_dataloader(boxes, image_id, dataloader, cfg_visualization, class_ids=None):
    print("Showing detections for image {0}, dataset {1}".format(
        image_id, dataloader.get_name()))
    image_to_show = get_image_from_dataloader(image_id, dataloader)
...
def show_detections(boxes, image_to_show, class_images,
                    cfg_visualization, image_names=None,
    ...
    fig, detected_list = show_annotated_image(img=image_to_show,
                                              boxes=boxes,
    ...
    return fig, detected_list


def show_gt_boxes(image_id, gt_boxes, class_ids, dataloader, image_to_show=None):
    print("Showing all GT boxes for image {0}, dataset {1}".format(
        image_id, dataloader.get_name()))
...
def get_image_from_dataloader(image_id, dataloader):
    img = dataloader._transform_image(
        image_id, do_augmentation=False, hflip=False, vflip=False)[0]
    img = dataloader.unnorm_image(img)
    return img


def show_annotated_image(img, boxes, labels, scores, class_ids, class_images, score_threshold=0.0, image_names=None,
    ...
    if good_ids.numel() > 0:
        ...
        # Here modification is done by prototype development team , the modification done is changing the captions
        # modify start
        print("Image names")
        print(image_names)

        label_names = [Path(image_names[l.item()][0]).name for l in labels]
        print("Iam in visulaization")
        print(label_names)
        # modify end
    ...
    if default_boxes is not None:
        ...
        scores = torch.cat([torch.Tensor(len(default_boxes)).to(
            scores).fill_(float("nan")), scores], 0)
    ...
    if transform_corners is not None:
        ...
        #results = colour_histogram(img=img,in_img = in_img,cl_img = cl_img,boxes = boxes)
        #res = kmeans(img=img,in_img = in_img,cl_img = cl_img,boxes = boxes)
        #res1 = colorogram(img=img,in_img = in_img,cl_img = class_image,boxes = boxes)
        #results = colorific(img=img,in_img = in_img,cl_img = class_image,boxes = boxes)
        fig, detected_string = vis_image(img,
                                         showfig=showfig,
                                         boxes=boxes,
                                         scores=scores,
...
def colour_histogram(img, in_img, cl_img, boxes):
    ...
    for i in range(boxes.size()[0]):
        ...
        im1 = in_img[int(boxes[i][1].item()):int(boxes[i][3].item()), int(
            boxes[i][0].item()):int(boxes[i][2].item())]
        #a = cv2.cvtColor(im1, cv2.COLOR_BGR2RGB)
        hist = cv2.calcHist([im1], [0, 1, 2], None, [
                            8, 8, 8], [0, 256, 0, 256, 0, 256])
        hist = cv2.normalize(hist, hist).flatten()
        histograms_in.append(hist)

    c = cv2.calcHist([cl_img], [0, 1, 2], None, [
                     8, 8, 8], [0, 256, 0, 256, 0, 256])
    ...
    for i in range(len(histograms_in)):
        d = cv2.compareHist(
            histograms_cl[0], histograms_in[i], cv2.HISTCMP_BHATTACHARYYA)
        results.append(d)
...
def colorogram(img, in_img, cl_img, boxes):
    ...
    results = []
    ans = []
    d = img.crop((int(boxes[7][0].item()), int(boxes[7][1].item()), int(
        boxes[7][2].item()), int(boxes[7][3].item())))
    d.show()
    for i in range(boxes.size()[0]):
        c = img.crop((int(boxes[i][0].item()), int(boxes[i][1].item()), int(
            boxes[i][2].item()), int(boxes[i][3].item())))
   ...
   # print(rgb1)
    ...
    while (i <= len(results)-3):
        p1 = np.array([float(results[i][0]), float(
            results[i][1]), float(results[i][2])])
        p2 = np.array([rgb1[0][0], rgb1[0][1], rgb1[0][2]])
        dist1 = np.sum((p1-p2)**2, axis=0)
        dist1 = np.sqrt(dist1)

        p3 = np.array(
            [float(results[i+1][0]), float(results[i+1][1]), float(results[i+1][2])])
        p4 = np.array([rgb1[1][0], rgb1[1][1], rgb1[1][2]])
        dist2 = np.sum((p3-p4)**2, axis=0)
        dist2 = np.sqrt(dist2)

        p5 = np.array(
            [float(results[i+2][0]), float(results[i+2][1]), float(results[i+2][2])])
...
#Crop a particular section
    ...
    d = img.crop((int(boxes[6][0].item()), int(boxes[6][1].item()), int(
        boxes[6][2].item()), int(boxes[6][3].item())))
    width, height = d.size
    ...
    for i in range(boxes.size()[0]):
        p1 = np.array([float(in_res[i][0]), float(
            in_res[i][1]), float(in_res[i][2])])
