...
# Output
# 1. base64 of generated detected_image
# 2. Outputs the items identified in detected_image ex : [choclate , choclate , waffy, waffy ,choclate ,waffy ,oreo ]
...
def get_prediction(path, list_of_tuple_for_detections_from_class_images):
    base64String = []
    class_images = []
    class_ids = []
    for fileDir, filePath in list_of_tuple_for_detections_from_class_images:
        class_images.append(read_image(filePath))
    class_ids = []
    for i in range(len(class_images)):
        class_ids.append(i)

    # input_image = read_image('data/demo/1.jpg')
    ...
    fig, list = visualizer.show_detections(boxes, input_image, class_images,
                                           cfg.visualization.eval, image_names=list_of_tuple_for_detections_from_class_images)

    # fig.savefig('bounding_box.png')
    #
